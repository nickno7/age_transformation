...
class MyConv2d(nn.Module):
    """Convolutional layer with equalized learning rate and custom learning rate multiplier."""
...
class StyleMod(nn.Module):
    ...
    def forward(self, x, latent):
        style = self.lin(latent) # [batch_size, n_channels * 2]
...
class GSynthesisBlock(nn.Module):
    """Generates a synthesis block with upscaling and convolutions."""

    def __init__(self, in_channels, out_channels, blur_filter, dlatent_size, gain, use_wscale, use_noise, use_pixel_norm, use_instance_norm, use_styles, activation_layer):
        super().__init__()

        # Blur layer is applied if the blur filter is provided
...
class G_synthesis(nn.Module):
    """Generative Synthesis Network for generating images from disentangled latents."""

    def __init__(self,
        dlatent_size        = 512,          # Latent vector size
        num_channels        = 3,            # Output color channels.
        resolution          = 1024,         # Output resolution. (1024x1024)
        fmap_base           = 8192,         # Feature Map Multiplier
