...
class Encoder(nn.Module):
    """Encoder network that uses ResNet50 as a backbone
        and learns to map images to latents."""
...
def check_and_generate_dataset(device, dataset_path=DATASET_PATH,
                               g_mapping=None, g_synthesis=None, num_samples=10000):
    ...
    if not os.path.exists(images_path) or len(os.listdir(images_path)) == 0 or len(os.listdir(latents_path)) == 0:
        ...
        generate_dataset(device, g_mapping, g_synthesis,
                         num_samples=num_samples, save_path=dataset_path)
    else:
...
def train(device, g_mapping, g_synthesis, dataset_dir=DATASET_PATH):
    ...
    check_and_generate_dataset(device, DATASET_PATH, g_mapping=g_mapping,
                               g_synthesis=g_synthesis, num_samples=10000)
    ...
    start_epoch, loss_history = load_checkpoint(device, encoder, optimizer,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        with tqdm(train_loader, desc=f"Epoch {epoch + 1}/{epochs}", unit="batch") as pbar:
            for imgs, true_latents in pbar:
                ...
                # Compute the loss between the generated and the real latent
                loss = logcosh_loss(true_latents, predicted_latents)
